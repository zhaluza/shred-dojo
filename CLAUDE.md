...
| `/metronome` | Standalone "Practice Station": circular beat-dial metronome (tap tempo, subdivisions, tempo-trainer auto-ramp) + countdown timer (presets/custom length/+5 min) + a Key & Drone card (circle-of-fifths key picker driving an optional tonic drone). The metronome + timer run in a root-level provider so they **persist across navigation** (the floating `MetronomeWidget` is their remote); see the Practice Station gotcha. Two-column metronome/timer row above the fold, drone card below. |
...
- **Practice Station engine — shared & persistent** (`practiceStation.tsx`). The standalone `/metronome` page's **metronome + tempo trainer + countdown timer** live in a **`PracticeStationProvider`** mounted at the App root (`root.tsx`, wrapping `<Outlet>` + `MetronomeWidget`), so a running session keeps clicking and counting down **across navigation** and stays loggable from any page. The page (`Metronome.tsx`) and the floating **`MetronomeWidget`** are both **views** into this one engine via `usePracticeStation()` → `{ met, ramp, timer }`; the widget is the compact **remote** (bpm/subdiv/tap/start-stop + live timer readout + the Save/Discard log prompt). `met-*` keys: `met-bpm/-sub/-timesig/-grouping/-accents-only/-vol`, tempo-trainer `met-ramp-on/amt/bars`, timer `met-timer-sec`, drone `met-drone-*`. The widget's tonic **drone** is an independent quick utility (its own AudioContext, `droneKey` local state — not the provider). Morning Coffee & Pentatonic Practice still embed their **own** inline metronomes (`useMetronomePanel`, keys `mc-*`/`pp-*`) and, like `/metronome`, suppress the floating widget (`root.tsx` checks `pathname`) — folding those two into the shared engine is the remaining cleanup.
- **Countdown timer** — logic lives in `Timer.tsx`'s **`useTimer`** hook (returns a `TimerController`); **`TimerView`** renders a supplied controller, and **`Timer`** is a self-contained wrapper (`storageKey`, `presets?`, `maxMin?`, `onSecond?`, `onLogSession?`, `onDiscardSession?`, `defaultLabel?`; also exports `fmtClock`) used by Pentatonic Practice (`pp-timer-sec`) and CAGED Immersion (`caged-timer-sec`). The Practice Station instead drives `TimerView` with the **provider's shared controller** (so the widget and page share one timer). It counts down off a **wall-clock `deadlineRef`** (not per-tick decrement) and resyncs on `visibilitychange`, so a backgrounded/throttled tab stays accurate; elapsed time for logging is accrued from real time (`Date.now()` deltas, pulsing `onSecond` per whole second), and `addFive` bumps `deadlineRef` directly. Don't reintroduce `remaining`-decrement-per-tick — it drifts in inactive tabs.
- **Drone (`Drone.tsx`)** — shared `useDrone(rootPc, storageKey)` hook + `DronePanel`, used by Pentatonic Practice (in the key aside, toggle key `d`, `pp-drone-vol`), the standalone Metronome page (Key & Drone card, `met-drone-vol`), and CAGED Immersion (`caged-drone-vol`). Own AudioContext, created lazily on first toggle (within the click gesture, for autoplay policy); sine root+chorus+fifth+octave voices that retune smoothly when the key changes. Starts **off** and only persists volume — an "on" state isn't restored since audio can't auto-resume without a gesture. The circle-of-fifths key picker is the shared `CircleKeySelector.tsx` (a compact picker; distinct from the full `/circle-of-fifths` info page). Both pull key data (`KEYS`/`NOTE_NAMES`/`mod12`) from `pentatonicPractice.utils.ts`.
